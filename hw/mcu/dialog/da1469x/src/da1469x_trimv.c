...
/* XXX Largest group id found in OTP was 18 so far */
#define TRIMV_GROUP_ID_MAX      18
...
static struct da1469x_trimv_group g_mcu_trimv_groups[TRIMV_GROUP_ID_MAX + 1];
...
da1469x_trimv_init_from_otp(void)
{
    ...
    while (oaddr < oaddr_end) {
        ...
        if ((ow == CS_WORD_END) || (ow == CS_WORD_INVALID)) {
            ...
            assert(trimv_group <= TRIMV_GROUP_ID_MAX);
            /*
             * XXX not sure if each group can be present only once in OTP, but
             *     our implementation requires it for now and it works
             */
